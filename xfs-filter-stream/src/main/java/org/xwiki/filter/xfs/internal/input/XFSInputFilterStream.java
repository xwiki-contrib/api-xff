...
public class XFSInputFilterStream extends AbstractBeanInputFilterStream<XFSInputProperties, XFSInputFilter>
{
    @Inject
    private Provider<XFSReader> xfsReaderProvider;
    ...
    @Override
    protected void read(Object filter, XFSInputFilter proxyFilter) throws FilterException
    {
        InputSource inputSource = this.properties.getSource();

        if (inputSource instanceof InputStreamInputSource) {
            XFSReader xfsReader = this.xfsReaderProvider.get();
            xfsReader.setProperties(this.properties);
            xfsReader.read(filter, proxyFilter);
        } else {
