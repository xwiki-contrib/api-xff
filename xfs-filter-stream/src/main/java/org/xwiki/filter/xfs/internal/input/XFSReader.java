...
import org.xwiki.filter.xfs.internal.UncloseableZipInputStream;

/**
 * @version $Id$
 * @since 7.1
 */
@Component(roles = XFSReader.class)
@InstantiationStrategy(ComponentInstantiationStrategy.PER_LOOKUP)
public class XFSReader
{
    /**
     ...
     */
    private XFSInputProperties properties;
     ...
     */
    public void setProperties(XFSInputProperties properties)
    {
    ...
    /**
     * Entry point for reading a XFS file.
     * 
     * @param filter is the input filter
     * @param proxyFilter is the filter into which you translate
     */
    public void read(Object filter, XFSInputFilter proxyFilter)
    {
        ...
        if (source instanceof FileInputSource) {
            try {
                ...
                if (isZip(inputFile)) {
                    parseXFSFile(inputFile, filter, proxyFilter);
                    ...
                    parseXFSDir(rootPath, path, filter, proxyFilter);
                } else {
                    this.logger.error("Don't know how to parse this kind of XFS format");
                }
            ...
            try {
                ...
                parseXFSInputStream(inputStream, filter, proxyFilter);
            } catch (FilterException e) {
            ...
            this.logger.error("Fail to read XFS file descriptor.");
        }
    }

    private void parseXFSFile(File file, Object filter, XFSInputFilter proxyFilter) throws IOException, FilterException
    {
    ...
    private void parseXFSInputStream(InputStream inputStream, Object filter, XFSInputFilter proxyFilter)
    ...
    private void parseXFSDir(Path rootPath, Path path, Object filter, XFSInputFilter proxyFilter) throws IOException,
        FilterException
    {
        ...
        if (file.isDirectory()) {
            ...
            for (String filename : subPaths) {
                ...
                parseXFSDir(rootPath, subPath, filter, proxyFilter);
            }
        }
